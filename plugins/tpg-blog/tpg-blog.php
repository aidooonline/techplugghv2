<?php
/**
 * Plugin Name: TechPlug GH Blog and Archive
 * Description: Adds a professional, native-looking blog and article archive to the TechPlug GH storefront. Activate and it creates a Guides and Reviews hub, adds it to the primary menu, and styles everything to match the Aurora theme. No theme edits.
 * Version: 1.0.0
 * Author: TechPlug GH
 * License: GPL-2.0-or-later
 * Text Domain: tpg-blog
...
add_action( 'wp_enqueue_scripts', function () {
	if ( tpgb_is_hub() || is_singular( 'post' ) ) {
		wp_enqueue_style( 'tpg-blog', TPGB_URL . 'assets/tpg-blog.css', array(), TPGB_VERSION );
	}
} );
...
add_filter( 'wp_nav_menu_items', function ( $items, $args ) {
	...
	$active = tpgb_is_hub() ? ' text-aur-blue' : '';
...
function tpgb_thumb( $size = 'large', $class = 'tpgb-thumb-img' ) {
	...
	if ( function_exists( 'tpg_placeholder_svg' ) ) {
		echo tpg_placeholder_svg( $class ); // phpcs:ignore
		return;
	}
