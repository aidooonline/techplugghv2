<?php
/**
 * TechPlug GH Blog and Archive hub template.
...
<div class="wrap py-14 tpgb">
	...
	<div class="tpgb-toolbar">
		<form role="search" method="get" class="tpgb-search" action="<?php echo esc_url( $tpgb_base_url ); ?>">
			...
			<input type="search" name="q" class="input tpgb-search-input" value="<?php echo esc_attr( $tpgb_q ); ?>" placeholder="<?php esc_attr_e( 'Search guides and reviews...', 'tpg-blog' ); ?>">
			<button type="submit" class="btn-primary tpgb-search-btn"><?php esc_html_e( 'Search', 'tpg-blog' ); ?></button>
		</form>

		<?php $tpgb_terms = tpgb_filter_terms( 8 ); ?>
		<?php if ( $tpgb_terms ) : ?>
		<nav class="tpgb-pills" aria-label="<?php esc_attr_e( 'Filter by topic', 'tpg-blog' ); ?>">
			<a class="chip tpgb-pill<?php echo ( ! $tpgb_tag ? ' tpgb-active' : '' ); ?>" href="<?php echo esc_url( $tpgb_base_url ); ?>"><?php esc_html_e( 'All', 'tpg-blog' ); ?></a>
	...
	<?php if ( '' !== $tpgb_q ) : ?>
		<p class="tpgb-resultline">
			...
			printf(
				...
				esc_html__( '%1$s result(s) for "%2$s"', 'tpg-blog' ),
				(int) $tpgb_query->found_posts,
				esc_html( $tpgb_q )
			);
			?>
			<a class="tpgb-clear" href="<?php echo esc_url( $tpgb_base_url ); ?>"><?php esc_html_e( 'clear', 'tpg-blog' ); ?></a>
		</p>
	<?php endif; ?>
	...
	<?php if ( $tpgb_query->have_posts() ) : ?>
				...
				<article class="card-glow tpgb-featured">
					...
					<div class="tpgb-featured-body">
						<div class="tpgb-meta">
							...
							<span><?php echo esc_html( $tpgb_rt ); ?> <?php esc_html_e( 'min read', 'tpg-blog' ); ?></span>
						</div>
						<a href="<?php the_permalink(); ?>"><h2 class="font-display font-semibold text-aur-paper tpgb-featured-title"><?php the_title(); ?></h2></a>
						<p class="tpgb-excerpt"><?php echo esc_html( wp_trim_words( get_the_excerpt(), 40 ) ); ?></p>
						<a href="<?php the_permalink(); ?>" class="menu-link text-aur-blue tpgb-readmore"><?php esc_html_e( 'Read the full guide', 'tpg-blog' ); ?> &rarr;</a>
					</div>
				</article>
				<?php
				continue;
			...
			<article class="card-glow tpgb-card">
				...
				<div class="tpgb-card-body">
					<div class="tpgb-meta">
						...
						<span><?php echo esc_html( get_the_date() ); ?></span>
						<span class="tpgb-dot">&middot;</span>
						<span><?php echo esc_html( $tpgb_rt ); ?> <?php esc_html_e( 'min', 'tpg-blog' ); ?></span>
					</div>
					<a href="<?php the_permalink(); ?>"><h2 class="font-display font-semibold text-aur-paper tpgb-card-title"><?php the_title(); ?></h2></a>
					<p class="tpgb-excerpt"><?php echo esc_html( wp_trim_words( get_the_excerpt(), 22 ) ); ?></p>
					<a href="<?php the_permalink(); ?>" class="menu-link text-aur-blue tpgb-readmore"><?php esc_html_e( 'Read more', 'tpg-blog' ); ?> &rarr;</a>
				</div>
			</article>
		<?php endwhile; ?>
		...
		if ( $tpgb_total > 1 ) :
			...
			if ( $tpgb_links ) {
				echo '<nav class="tpgb-pagination" aria-label="' . esc_attr__( 'Posts', 'tpg-blog' ) . '">' . $tpgb_links . '</nav>'; // phpcs:ignore
			}
		endif;
		?>
	...
	<?php else : ?>
		<div class="card-glow tpgb-empty">
			<p class="text-aur-paper font-display"><?php esc_html_e( 'Nothing matches that yet.', 'tpg-blog' ); ?></p>
			<p class="text-aur-muted"><?php esc_html_e( 'Try another topic, or browse everything.', 'tpg-blog' ); ?></p>
			<a class="btn-primary tpgb-empty-btn" href="<?php echo esc_url( $tpgb_base_url ); ?>"><?php esc_html_e( 'Browse all articles', 'tpg-blog' ); ?></a>
		</div>
